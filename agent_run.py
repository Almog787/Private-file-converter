...
def fetch_google_trends(geo: str = "US") -> str:
    ...
    try:
        ...
        # Extract the top 10 trending items
...
def main():
    ...
    # Initialize the Gemini model targeting the free tier
    model = GeminiModel(model_id="gemini-3.5-flash", api_key=api_key)

    # Initialize the CodeAgent with strict execution boundaries
    agent = CodeAgent(
        tools=[fetch_google_trends],
        model=model,
        max_steps=10, # CRITICAL: Prevents infinite loops and token exhaustion
        additional_authorized_imports=["os", "datetime", "json", "re"] # Authorized modules for file scanning
    )

    # The exact instructions defining the agent's behavior and constraints
    task_prompt = """
    ...
    print("Initiating repository scan and autonomous enhancement...")
